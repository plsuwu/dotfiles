...
myStartupHook = do
    spawn gnomePolkit
    spawn
        "xrandr --output DP-0 --mode 2560x1440 --pos 0x1440 --rate 170 --primary --output DP-4 \
        \--mode 2560x1440 --pos 0x0 --rate 165"
    spawn "xsetroot -cursor_name left_ptr &"
    spawn "systemctl --user import-environment DISPLAY"
    spawn "clipmenud"
    spawn $ "/home/please/.local/bin/eww -c " ++ confDir ++ "/eww open-many primary-bar secondary-bar"

    -- idk why trayer doesnt spawn - i assume ordering - but i am too lazy to try to figure this out atm.
    spawn
        "trayer --edge top --align right --SetDockType true --SetPartialStrut true --monitor 0 --width 10 \
        \--distance 1 --iconspacing 7 --expand false --tint 0xF4C6E0 --transparent true --alpha 0"
    spawnOn "0" "vesktop"
...
myManageHook =
  ...
  where
    {- fetch _NET_WM_STATE prop as list of atoms -}
